	...
	public static void main(String[] args) {
		...
		while (ch != 0) { // selama tidak memilih keluar
			if (1 == ch) { // fungsi 1
				...
				double hasil = p.distanceToLineSegment(l);// mendapatkan hasil dengan cara memanggil method untuk
															// mencari jarak dari segmen garis ke titik
				...
				if (res < 0.0)
					arah = "Kanan"; // kondisi jika posisi titik terhadap segmen garis belok kanan
				else if (res > 0.0)
					arah = "Kiri"; // kondisi jika posisi titik terhadap segmen garis belok kiri
				...
				MyLineSegment l = new MyLineSegment(p, q); // instansiasi segmen garis pertama dari titik pertama dan
															// titik kedua
				MyLineSegment m = new MyLineSegment(r, s); // instansiasi segmen garis kedua dari titik ketiga dan titik
															// keempat
				boolean intersect = l.isIntersect(m); // menentukan berpotongan atau tidak berpotongan dengan memanggil
														// method isIntersect untuk segmen garis pertama dan kedua
				...
				for (int i = 0; i < n; i++) {
					MyPoint p = new MyPoint(sc.nextDouble(), sc.nextDouble()); //instansiasi titik yang akan dimasukan
					poly.addPoint(p); //memasukan titik ke polygon

				}
				boolean hasil = poly.isConvex(); //memanggil method isConvex untuk menentukan Convex atau Concave

				if (hasil == true) { // kondisi saat semua segmen garis berbelok ke kiri(Convex)
					System.out.println("Convex");

				} else {
					System.out.println("Concave");// kondisi saat segmen garis ada yang berbelok ke kanan(Concave)
				}
				...
				for (int i = 0; i < n; i++) {
					MyPoint p = new MyPoint(sc.nextDouble(), sc.nextDouble()); //input titik
					poly.addPoint(p); //memasukan input titik ke dalam array list polygon
				...
				List points = mps.getPoints();
				for (int i = 0; i < n; i++) {
					Collections.sort(mps.Points, (mps.Points(i), mps.Points(i+1)) -> {
						double cr = CG.cross(mps.Points.get(i).x - mps.carititikTerkecil().x,
								(mps.Points.get(i)).y - mps.carititikTerkecil().y,
								mps.Points.get(i + 1).y - mps.carititikTerkecil().y,
								mps.Points.get(i + 1).y - mps.carititikTerkecil().y);
						if (cr > 0) {
							return 1;
						} else {
