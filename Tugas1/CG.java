public class CG {
    ...
    public static double dot(MyPoint p, MyPoint q, MyPoint r) {
        MyPoint pq = new MyPoint((q.x-p.x),(q.y-p.y)); //instansiasi vector pq
        MyPoint pr = new MyPoint((r.x-p.x),(r.y-p.y)); //instansiasi vector pr
        double res = ((pq.x*pr.x)+(pq.y*pr.y)); //rumus dot product

        return res; //return dot product
    }
    ...
    public static double cross(double x1,double y1,double x2,double y2){
        return x1*y2 - x2*y1; //return cross product
    }
}
